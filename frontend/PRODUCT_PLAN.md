...
Replaced secondary text-only loading states with consistent skeleton rows for patients, requests, visits and reviews. Page placeholders now match list layouts; record retrieval metadata appears only when available. Downtime has a dedicated skeleton, loading announcements remain screen-reader accessible, and skeleton motion respects reduced-motion preferences. Verified formatting, lint, TypeScript, 55 component/API tests and all 15 browser journeys against a production build. React Doctor: 93/100, two existing list-complexity warnings.
