...
describe("emergency session contract flow", () => {
  ...
  test("lets the source or recipient security administrator revoke the session", async () => {
    await login({ username: "amina.unity", password: "synthetic-example-password" });
    const activated = await activateEmergency({
      patient_id: DEMO_PATIENT_ID,
      source_org_id: DEMO_MERCY_ORGANIZATION_ID,
    ...
    const revoked = await revokeEmergency({
      sessionId: activated.session.id,
      input: {
        expected_version: activated.session.version,
        reason: "Security review found that this emergency scope is no longer necessary.",
      },
    });
    ...
    await login({ username: "amina.unity", password: "synthetic-example-password" });
    await expect(
      getEmergencyRecords({ sessionId: activated.session.id, view: "summary" }),
    ).rejects.toMatchObject({ code: "EMERGENCY_REVOKED", status: 403 });
  });
});
