...
const domainMeta: Record<Domain, DomainMeta> = {
  demographics: {
    label: "Demographics",
    shortLabel: "Identity",
            ...
            <div className="flex flex-wrap items-start justify-between gap-5">
              <div className="flex items-start gap-4">
                <div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-primary text-primary-foreground shadow-sm">
                  <StethoscopeIcon aria-hidden="true" className="size-6" weight="duotone" />
                </div>
                <div>
                  ...
                  <h1 className="mt-2 font-heading text-2xl font-semibold tracking-tight sm:text-3xl">
                    {overview.isPending ? "Loading patient" : (patient?.name ?? "Patient record")}
                  </h1>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 px-5 pb-5">
            <div className="flex items-center justify-between gap-3 text-sm">
              ...
              <span className="font-medium">{source?.name ?? context.organization.name}</span>
            </div>
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="text-muted-foreground">Mode</span>
              <span className="font-medium">
                {source?.mode === "LITE" ? "Lite EMR" : "Existing EMR"}
              </span>
            </div>
            ...
            {isExchangePractitioner(context.role) ? (
              <Link
      ...
      <section className="mt-7 grid gap-6 lg:grid-cols-[13rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-5">
          ...
          {selected.isPending ? (
            ...
            <RecordErrorState error={selected.error} />
          ...
          {canWrite && encounterId && selectedDomain === "nursing_notes" ? (
            <NursingNoteComposer patientId={patientId} encounterId={encounterId} />
          ) : null}
          {canWrite && encounterId && selectedDomain === "vitals" ? (
            <VitalComposer patientId={patientId} encounterId={encounterId} />
          ) : null}
          {canWrite &&
