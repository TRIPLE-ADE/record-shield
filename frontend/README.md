...
- `next.config.ts` redirects the root path to sign in. Other route files stay thin and compose feature pages from the design-system, auth, workspace, patient-records, exchange, emergency, portal, security, administration, and downtime modules.
...
- React Query queries and mutations live in the global `hooks/` folder, grouped by domain (`auth.ts`, `patient-records.ts`, `exchange.ts`, `emergency.ts`, `security.ts`, `admin.ts`, and `downtime.ts`). They call feature API functions and own cache policy.
